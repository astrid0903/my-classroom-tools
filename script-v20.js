...
const els = {
  ...
  postBoardQrPopover: document.querySelector("#post-board-qr-popover"),
  ...
  postBoardNoteWrap: document.querySelector(".post-board-note-wrap"),
  postBoardNoteToggle: document.querySelector("#post-board-note-toggle"),
  postBoardActions: document.querySelector(".post-board-actions"),
...
let fileSaveLock = Promise.resolve();
...
async function scanPlaybackFolder({ silent = false } = {}) {
  ...
  if (!(await hasHandlePermission(playbackDirectoryHandle, { mode: "read" }))) {
    folderLayoutFiles = [];
    setFolderStatus("已記住上次的本機資料夾，請按「本機資料夾」按鈕啟用存取權限。");
    renderHomeVersions();
    return;
  }
...
async function choosePlaybackFolder() {
  ...
  // 若已有記憶的資料夾，且目前 session 尚未取得授權 (例如 prompt)，則先請求授權
  if (playbackDirectoryHandle && !(await hasHandlePermission(playbackDirectoryHandle, { request: false, mode: "read" }))) {
    try {
      setFolderStatus("正在請求本機資料夾讀取權限...");
      if (await hasHandlePermission(playbackDirectoryHandle, { request: true, mode: "read" })) {
        await scanPlaybackFolder();
        return;
      }
    } catch (error) {
      console.warn("啟用記憶資料夾授權失敗，將嘗試重新選擇資料夾：", error);
    }
  }
  ...
  if (playbackDirectoryHandle && directoryAccessSupported()) {
    try {
      if (!(await hasHandlePermission(playbackDirectoryHandle, { request: true }))) {
        setFolderStatus("未取得資料夾存取權限。請按「本機資料夾」啟用授權或重新選取資料夾。", true);
        setVersionMessage("未取得資料夾存取權限，無法建立新播放台檔案。", true);
        return;
      }
...
function initHomeMode() {
  ...
  // 1. 自動載入本機播放台資料夾預覽
  restorePlaybackFolder().catch(() => {});

  // 2. 自動載入上次的 Google Drive 資料夾預覽
  const hasPriorGrant = window.localStorage.getItem(GOOGLE_DRIVE_AUTH_GRANTED_KEY) === "true";
  if (hasPriorGrant) {
...
function syncTopControlTogglePlacement() {
  const page = activePage();
  const toggle = document.getElementById("top-control-toggle");
  const topControl = document.getElementById("top-control");
  
  if (!toggle) return;

  if (page && page.type === "posts") {
    if (els.postBoardActions && !els.postBoardActions.contains(toggle)) {
      els.postBoardActions.appendChild(toggle);
    }
  } else {
    if (topControl && !topControl.contains(toggle)) {
      topControl.insertBefore(toggle, topControl.firstChild);
    }
  }
}
...
async function saveCurrentDriveFile({ silent = false } = {}) {
  ...
  try {
    ...
    if (currentDriveFileId !== fileId) return false;
    // 寫回後一定要記「寫入後」的 modifiedTime。若上傳回應沒帶，就重讀一次；
    // 退回去記寫入前的 metadata.modifiedTime 會讓之後每次存檔都誤判衝突，而且自己好不了。
    let nextModifiedTime = updated.modifiedTime || "";
    if (!nextModifiedTime) {
      try {
        const refreshed = await getDriveFileMetadata(updated.id || fileId);
        nextModifiedTime = refreshed.modifiedTime || "";
      } catch (metaError) {
        nextModifiedTime = "";
      }
    }
    writeFileMeta({
      ...
      autoSave: true,
      driveFileId: updated.id || fileId,
      driveModifiedTime: nextModifiedTime,
    });
    setFileInputs(currentFileName);
    setFileStatusInputs();
    const folderItem = folderLayoutFiles.find((item) => item.driveFileId === fileId);
...
async function saveCurrentFile() {
  // 手動存檔也要排進存檔隊伍，否則會跟進行中的自動存檔交錯，造成假的「雲端有較新版本」警告。
  clearTimeout(fileSaveTimerId);
  await withFileSaveLock(performSaveCurrentFile);
}

async function performSaveCurrentFile() {
...
// 所有存檔動作（自動存檔、手動按「存檔」）都排進同一條隊伍，一次只跑一個。
// 兩個寫入交錯時，後跑的那個會讀到前一個剛寫進 Drive 的 modifiedTime，
// 誤判成「雲端有較新的版本」而跳出衝突警告——其實那是本機自己一秒前寫的。
function withFileSaveLock(task) {
  const run = fileSaveLock.then(task, task);
  fileSaveLock = run.then(() => {}, () => {});
  return run;
}
...
async function autoSaveCurrentFile() {
  ...
  try {
    await withFileSaveLock(performAutoSaveCurrentFile);
  } finally {
...
async function performAutoSaveCurrentFile() {
...
function setPostBoardQrOpen(isOpen) {
  ...
  els.postBoardQrPopover.classList.toggle("hidden", !isOpen);
  els.postBoardQrToggle.classList.toggle("active", isOpen);
  els.postBoardQrToggle.setAttribute("aria-expanded", String(isOpen));
}
...
els.postBoardQrToggle.addEventListener("click", (e) => {
  ...
  const shouldOpen = els.postBoardQrPopover?.classList.contains("hidden");
  setPostBoardQrOpen(Boolean(shouldOpen));
  if (shouldOpen && els.postBoardNoteWrap && !els.postBoardNoteWrap.classList.contains("hidden")) {
    els.postBoardNoteWrap.classList.add("hidden");
  }
});
if (els.postBoardQrPopover) {
  els.postBoardQrPopover.addEventListener("click", (event) => event.stopPropagation());
}
...
// 連結按鈕
document.querySelectorAll(".note-link-btn").forEach((btn) => {
  btn.addEventListener("mousedown", (e) => {
    e.preventDefault();
    ...
    const url = window.prompt("請輸入超連結網址 (URL):", defaultUrl);
    if (url) {
      if (selection.rangeCount > 0 && selectedText) {
        document.execCommand("createLink", false, url);
      } else {
        const linkText = window.prompt("請輸入連結文字 (選填):", url);
        const html = `<a href="${url}" target="_blank" rel="noopener noreferrer">${linkText || url}</a>`;
...
document.querySelectorAll(".color-swatch").forEach((swatch) => {
  swatch.addEventListener("mousedown", (e) => {
    ...
    els.postBoardNoteColor.value = color;
...
// 公告 Popover 收合控制
if (els.postBoardNoteToggle) {
  els.postBoardNoteToggle.addEventListener("click", (e) => {
    e.stopPropagation();
    if (els.postBoardNoteWrap) {
      els.postBoardNoteWrap.classList.toggle("hidden");
      if (!els.postBoardNoteWrap.classList.contains("hidden")) {
        els.postBoardNote.focus();
        // 互斥：收合 QR Code popover
        setPostBoardQrOpen(false);
      }
    }
  });
}
document.addEventListener("click", (e) => {
  if (els.postBoardNoteWrap && !els.postBoardNoteWrap.classList.contains("hidden")) {
    const isClickInside = els.postBoardNoteWrap.contains(e.target) || 
                          (els.postBoardNoteToggle && els.postBoardNoteToggle.contains(e.target));
    if (!isClickInside) {
      els.postBoardNoteWrap.classList.add("hidden");
    }
  }
...
document.addEventListener("click", (e) => {
  ...
  if (els.postBoardQrPopover && !els.postBoardQrPopover.classList.contains("hidden")) {
    const isClickInside = els.postBoardQrPopover.contains(e.target) ||
